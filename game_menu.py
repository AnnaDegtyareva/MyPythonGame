...
class TopPanel():
    ...
    def finish_screen(self, x, y):

        self.font_finish = pygame.font.SysFont("Comic sans", 50)
        ...
        # Создаем надпись для количество собранных монет:
        self.coin_rect.x = x + 200
        self.coin_rect.y = y - 78

        # Создаем пиктограмму монеты:
        self.coin_img_rect.y = y - 80
        self.coin_img_rect.x = x + 165

        self.timer.rect.x = x + 350
        self.timer.rect.y = y - 45

    def finish_screen_draw(self, screen):

        # Создаем и отображаем надпись
        self.label_1 = self.font_finish.render('CONGRATILATIONS' , True, WHITE)
        screen.blit(self.label_1, [WIN_WIDTH // 2.6, WIN_HEIGHT // 12.2])
        self.label_2 = self.font_finish.render('Количество набранных очков: ', True, WHITE)
        screen.blit(self.label_2, [WIN_WIDTH // 16, WIN_HEIGHT // 2.3])
        self.label_2 = self.font_finish.render('Количество времени прохождения игры: ', True, WHITE)
        screen.blit(self.label_2, [WIN_WIDTH // 16, WIN_HEIGHT // 2.0])
        self.label_2 = self.font_finish.render('Количество жизней: ', True, WHITE)
        screen.blit(self.label_2, [WIN_WIDTH // 16, WIN_HEIGHT // 1.75])
    ...
    def game_over_screen_draw(self, screen):

        # Создаем и отображаем надпись
        self.label_1 = self.font_finish.render('GAME OVER' , True, WHITE)
        screen.blit(self.label_1, [WIN_WIDTH // 2.6, WIN_HEIGHT // 12.2])
        self.label_2 = self.font_finish.render('Количество набранных очков: ', True, WHITE)
        screen.blit(self.label_2, [WIN_WIDTH // 16, WIN_HEIGHT // 2.3])
        self.label_2 = self.font_finish.render('Количество времени прохождения игры: ', True, WHITE)
